			...
			@tap.prevent.stop="voidFc()">
				<view class="flex_column" :class="(mode||'middle') + '_view'" :style="{'height': heightSize}">
					<block v-if="mode==='bottom'">
						<view class="flex_row_between_c width100 padding20rpx" style="background:#F8F8F8 ;" :style="{'font-size': titleSize, 'background-color': bgColor_title||'#f8f8f8'}">
					...
					<block v-else-if="mode==='middle'">
						<view class="flex_row_c_c width100 padding20rpx" style="background:#F8F8F8 ;" :style="{'font-size': titleSize, 'background-color': bgColor_title||'#f8f8f8', 'color': titleColor||'#999'}" v-if="title">
							{{title||''}}
						</view>
					</block>
...
<script>
	...
	export default {
		...
		methods: {
			init() {
				...
				switch (this.type) {
					case 'date': //date
						...
						setObj.startYear=1900
						years = _app.countYears(setObj.startYear || (new Date().getFullYear() - 100), setObj.endYear || (new Date().getFullYear() - 1));
							console.log(setObj.startYear)
			...
			_getCityCode() {
				let pcikerLabel =[this.provinceDataList[this.value[0]].value,this.cityDataList[this.value[1]].value,this.areaDataList[this.value[2]].value]
				return pcikerLabel;
			},
			show() {
